...
|---|---|---|---|
| `03_blazor/HalloBlazor` | 03 | Minimale Blazor Web App (Interactive Server): Eingabe, Button, Ausgabe mit `@bind`/`@onclick` | `dotnet run --project 03_blazor/HalloBlazor`, dann die angezeigte URL im Browser öffnen |
| `03_blazor/Geometrieeditor` | 02, 03, 08, 12 | Das durchgehende Beispiel: Solution mit `Fachkonzept` (Figur-Hierarchie, `FigurenVerwaltung`, `IFigurSpeicher`), `Datenhaltung` (Arbeitsspeicher- und JSON-Speicher), `Web` (Blazor: Seite mit Figurenliste, Details und modalem Dialog als Komponente) und `Tests` (NUnit) | `dotnet run --project 03_blazor/Geometrieeditor/Geometrieeditor.Web` und `dotnet test 03_blazor/Geometrieeditor` |
...
```bash
cd examples
for p in 03_blazor/HalloBlazor 03_blazor/Geometrieeditor 10_pinvoke/PInvokeDemo 11_nuget/NLogDemo 12_unittests/Bruch; do
  dotnet build "$p" || exit 1
done
dotnet test 03_blazor/Geometrieeditor
