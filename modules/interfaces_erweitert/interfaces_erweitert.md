...
{
    string Name { get; }
    double Menge { get; set; }
}

interface IHeissgetraenk : IGetraenk
{
...
Eine Klasse, die `IHeissgetraenk` implementiert, verpflichtet sich damit automatisch auch zu `Name` und `Menge`. Und jedes `IHeissgetraenk`-Objekt lässt sich einer `IGetraenk`-Variablen zuweisen – dieselbe Ersetzbarkeit wie bei Basisklasse und Unterklasse, nur ohne geerbten Code. Interfaces dürfen dabei von mehreren Interfaces erben, was Klassen mit Basisklassen verwehrt bleibt.
...
class Tee : IHeissgetraenk
{
    ...
    void IHeissgetraenk.Abkuehlen(int grad)
    {
        Temperatur -= grad;
    }
}
...
```csharp
Tee tee = new Tee { Temperatur = 95 };
tee.Abkuehlen(10);                        // Compilerfehler: Tee hat kein Abkuehlen
((IHeissgetraenk)tee).Abkuehlen(10);      // funktioniert
IHeissgetraenk h = tee;
h.Abkuehlen(10);                          // funktioniert
Console.WriteLine(tee.Temperatur);        // 75
```
...
interface IHeissgetraenk : IGetraenk
{
    ...
    bool IstTrinkbar => Temperatur < 65;
}

class Kaffee : IHeissgetraenk, IHatKoffein
{
    public string Name => "Kaffee";
    public double Menge { get; set; }
    public int Temperatur { get; set; }
    public double Koffein { get; set; }
    // Abkuehlen und IstTrinkbar kommen aus dem Interface
}
```

Der Hauptzweck ist, ein Interface nachträglich um eine Methode erweitern zu können, ohne alle existierenden Implementierungen zu brechen – bei Bibliotheken mit vielen Nutzern ein echtes Problem. Zwei Dinge sollte man wissen: Default-Mitglieder verhalten sich wie explizit implementierte, sind also nur über den Interface-Typ erreichbar (`kaffee.IstTrinkbar` geht nicht, `((IHeissgetraenk)kaffee).IstTrinkbar` schon). Und ein Interface bleibt trotzdem zustandslos – es kann keine Felder anlegen, sondern nur mit den Properties arbeiten, die es selbst deklariert.
...
Ein Punkt, der gern übersehen wird: Eine Methode, die ein Interface implementiert, ist **nicht automatisch virtuell**. Beim Interface selbst ist der Aufruf polymorph – über `IHeissgetraenk` landet man immer bei der Klasse, die das Interface implementiert. Aber wenn von dieser Klasse wiederum eine Unterklasse erbt und die Methode überschreiben will, gilt dieselbe Regel wie in [`virtual` und `override`](/modules/virtual_override/virtual_override.md):

```csharp
class Kaffee : IHeissgetraenk
{
    public int Temperatur { get; set; }
    public virtual void Abkuehlen(int grad) => Temperatur -= grad;   // virtual!
}

class Espresso : Kaffee
{
    public override void Abkuehlen(int grad) => Temperatur -= grad / 2;   // Tasse hält warm
}
```

Ohne das `virtual` in `Kaffee` gäbe es kein `override` in `Espresso` – bestenfalls ein `new`, mit dem Ergebnis, dass `IHeissgetraenk h = new Espresso()` beim Aufruf von `h.Abkuehlen(...)` die Kaffee-Version verwendet. Wer also Interface-Implementierungen in einer Klassenhierarchie weiter verfeinern will, markiert sie als `virtual` (oder `abstract`, wenn die Klasse selbst abstrakt ist).
